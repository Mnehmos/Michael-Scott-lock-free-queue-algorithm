...
inline ThreadData* HazardPointerManager::get_local_data() {
...
inline void* HazardPointerManager::acquire(int index, void* ptr) {
...
inline void HazardPointerManager::release(int index) {
...
inline void HazardPointerManager::retire_node(void* ptr) {
    ThreadData* local_data = get_local_data();
    local_data->retire_list.push_back(ptr);
    if (local_data->retire_list.size() >= R) {
        scan_and_reclaim();
    }
}

inline void HazardPointerManager::scan_and_reclaim() {
    ThreadData* local_data = get_local_data();
